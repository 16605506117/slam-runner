...
## SC-PGO + 水面高度先验（2026-08-11）

| 数据集 | 关键帧 | ATE 2D RMSE (m) | ATE 3D RMSE (m) | 水高因子 | 说明 |
|---|---|---|---|---|---|
| W06 | 143 | 0.215 | **0.304** | 142 | SC-PGO 无回环；z 先验降 3D 28% |
| N03 | 179 | 0.483 | **0.627** | 178 | z 先验降 3D 60% |
| H05 | 247 | 0.668 | **0.836** | 246 | z 先验降 3D 90% (8.32→0.84) |

- **方案 B（水面高度先验）**：SC-PGO 位姿图注入 z=0 先验因子（GPSFactor，xy 噪声 1e9 不约束、z 方差 0.05），H05 z 漂移从 ±7m 收敛到 ±0.27m
- 对比：无先验时 3D ATE W06 0.42 / N03 1.56 / H05 8.32；加先验后 0.30 / 0.63 / 0.84
- 2D ATE 接近无先验（z 约束对 xy 微耦合）；水面轨迹无回环，SC-PGO 回环增益待有闭环数据验证
