...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  int ts_unit = 0;
  nh.param<int>("preprocess/timestamp_unit", ts_unit, 0);  // 0:s 1:ms 2:us 3:ns (water/KITTI bag = 0)
  if (ts_unit == 1) p_pre->time_unit_scale = 1.0;
  else if (ts_unit == 2) p_pre->time_unit_scale = 0.001;
  else if (ts_unit == 3) p_pre->time_unit_scale = 0.000001;
  else p_pre->time_unit_scale = 1000.0;
