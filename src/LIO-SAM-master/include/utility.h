...
// flann 1.9.1 缺少 std::unordered_map 的序列化特化（上游在 1.9.2 修复）。
// 在定义 PCL_NO_PRECOMPILE 并实例化 KdTreeFLANN 时，flann 会为 LshIndex 的
// 虚函数生成代码，触发 "std::unordered_map has no member named serialize" 编译错误。
// 此处补上与 flann 1.9.2 上游一致的实现（须位于 kdtree_flann.h 之后）。
